...
public class MainController {
	...
	String food[] = {"국밥", "굶어라", "김밥", "냉면", "돈까스", "떡볶이","삼겹살","초밥","치킨","햄버거"};
	List<String> foodList = Arrays.asList(new String[] {
			"국밥", "굶어라", "김밥", "냉면", "돈까스", "떡볶이","삼겹살","초밥","치킨","햄버거" });
	...
	public void initialize() {
		// food[] 배열데이터를 리스트뷰에 초기화
		myListView.getItems().addAll(foodList);

		// 리스트뷰의 항목 선택 이벤트
		myListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {

			// 선택 항목이 변경됐을떄 실행되는 메소드
			@Override // ChangeListener 인터페이스, 익명객체 생성한것
			public void changed(ObservableValue<? extends String> arg0, String arg1, String arg2) {
				// 선택한 항목을 String currentFood; 에 지정
			...
			public void handle(MouseEvent event) {
				...
				if( event.getClickCount() == 2 ) {
					...
					try {
						...
						//화면 이동만
						//Main.setRoot("Detail");		//Detail.fxml화면으로 이동
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
